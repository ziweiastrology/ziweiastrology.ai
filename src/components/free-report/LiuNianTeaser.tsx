...
const LIU_NIAN_TOPICS = [
  ...
  },
  {
    ...
    primaryPalaceCn: "田宅宫",
    primaryPalaceEn: "Property Palace",
    prompt: "Analyze my 2026 family 流年 based on my 田宅宫 and 父母宫. What changes or focus areas do you see?",
  },
];

function buildTeaser(palace: PalaceDetail | undefined, palaceName: string, isZh: boolean): string {
  if (!palace) return `Your ${palaceName} holds unique patterns worth exploring...`;
  ...
  return `Your ${palaceName} holds ${mainStar}${stateLabel} — ${narratives[palace.state] || narratives.neutral}...`;
}
    ...
    <div className="relative p-8 sm:p-10 rounded-xl border border-gold-500/20 bg-gradient-to-b from-celestial-900/90 to-celestial-800/50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {LIU_NIAN_TOPICS.map((topic) => {
          ...
          const palaceName = isZh ? topic.primaryPalaceCn : topic.primaryPalaceEn;
          const teaser = buildTeaser(palace, palaceName, isZh);

          return (
            <button
              key={topic.key}
              onClick={() => handleCardClick(topic.prompt)}
              className="relative group p-5 pb-10 rounded-lg border border-gold-700/20 bg-celestial-900/60 text-left overflow-hidden
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-sm font-semibold text-parchment-200">
                    {isZh ? topic.labelCn : topic.labelEn}
                  </p>
                </div>
